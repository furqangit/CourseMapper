app.controller('PeerAssessmentController', function ($scope, $http, courseService, toastr, $window, Upload, $location, ActionBarService) {
    ...
    $scope.addCalibration = function (peerReviewId) {
        window.document.location = '#/cid/' + $scope.course._id + '/?tab=peerAssessment&pRId=' + peerReviewId + '&vName=addCalibration';
    }

    $scope.editCalibration = function (peerReviewId, calibrationId) {
    ...
    $scope.initiateView = function () {
        ...
        if ($scope.vName) {
            ...
            } else if ($scope.vName == 'addCalibration') {
                $scope.currentView = 'addCalibration.tpl'
                $scope.manageBreadCrumb('Add Calibration')
